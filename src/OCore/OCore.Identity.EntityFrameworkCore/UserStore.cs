...
using OCore.Identity.EntityFrameworkCore.Properties;

namespace OCore.Identity.EntityFrameworkCore
{
    public class UserStore:UserStoreBase<User,Role, string, UserClaim, UserRole, UserLogin, UserToken, RoleClaim>,IUserActiveStore<User>
    {
        public UserStore(AppDbContext context, IdentityErrorDescriber describer = null) : base(describer ?? new IdentityErrorDescriber())
        {
            if (context == null)
            {
        ...
        public virtual Task<bool> IsActiveAsync(User user, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            if (user == null)
        ...
        public virtual Task SetActiveAsync(User user, bool active, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            if (user == null)
