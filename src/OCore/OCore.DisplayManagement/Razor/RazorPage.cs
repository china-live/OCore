...
namespace OCore.DisplayManagement.Razor
{
    public abstract class RazorPage2<TModel> : Microsoft.AspNetCore.Mvc.Razor.RazorPage<TModel>
    {
        public string Theme { get; set; }

        public string GetTheme(string name)
        {
            var _extensionManager = Context.RequestServices.GetService<IExtensionManager>();
            return _extensionManager.GetExtension(name)?.SubPath;
        }
    }
    ...
    //                var _extensionManager = Context.RequestServices.GetService<IExtensionManager>();
    //                var s = _extensionManager.GetExtension("TheAdmin"); ;
    //            }
